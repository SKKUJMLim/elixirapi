defmodule HelloApiWeb.ReplyControllerTest do
  ...
  def fixture(:reply) do
    {:ok, reply} = Scheme.create_reply(@create_attrs)
    reply
  end

  describe "index" do
    test "lists all reply", %{conn: conn} do
      conn = get(conn, Routes.reply_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing Reply"
    end
  end
